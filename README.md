...
5. **운영진**: `/admin/categories`에서 카테고리(예: 웹개발/앱개발/게임개발) 추가 후, 각 카테고리를
   클릭해 채널 구성을 만든다. 텍스트/음성 채널을 원하는 만큼 추가할 수 있고, 정확히 1개 채널만
   "참가 신청 채널"로 지정하면 거기에 정원/참가 버튼이 붙는다 (문구 비우면 빈 채널로 생성).
6. **운영진**: `/admin/competitions/new`에서 대회 등록 시 카테고리 선택 + 카테고리별 선착순 정원
   입력 → **대회명으로 된 디스코드 카테고리가 자동 생성**된다. 이 카테고리는 **비공개**라 대회
   역할을 가진 사람(=참가 성공한 사람)만 볼 수 있고, 카테고리에 속한 "참가 신청 채널이 아닌"
   채널들이 그 안에 생성된다. "참가 신청 채널"(참가 버튼 달린 안내)만은 아직 역할이 없는
   사람도 봐야 하므로 카테고리 밖(서버 최상위)에 공개로 생성된다. 대회 상세 페이지에서 언제든
   삭제 가능 (디스코드 카테고리/채널/역할도 함께 정리됨).
7. **부원**: 참가 신청 채널의 버튼 클릭 → 인증 여부/정원 확인 → 성공 시 대회명과 같은 이름의
   역할이 자동 부여되어 비공개 카테고리가 보이게 되고, 개인 DM으로 참가 완료 + 적립 포인트
   안내, 임베드의 참가 인원 수 갱신 (정원 도달 시 버튼 비활성화).
